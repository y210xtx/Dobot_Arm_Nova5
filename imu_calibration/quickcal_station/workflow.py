...
QUICKCAL_STEPS = (
    ...
    QuickCalStep("M01", "磁标定", "J2/J3/J4 与 J6 叠加覆盖", "固定 J1/J5 和 TCP XYZ，用约束正解生成 J2/J3/J4 俯仰，同时叠加 J6 ±75° 相移往复，再以绝对关节 MovJ 联动", "从标定中位开始；磁环境和线束已确认", 0, 0, 40, 0, 0x30, MCAL_CAPTURE_MAG, "掌部磁传感器原始场", "40 秒内四轴正反向覆盖达标；结束位置不限，阶段后独立回中；J1/J5 保持，TCP XYZ 漂移不超限"),
    QuickCalStep("M02", "磁标定", "Yaw 正向单侧往返", "Tool Rx 以 10.5°/s 从 0° 到 +45°，实测外端制动行程后沿原路径回到 0°", "从标定中位开始；路径无碰撞", 0, 0, 10, 0, 0x31, MCAL_CAPTURE_MAG, "掌部磁传感器原始场", "10 秒内完成正向单侧往返并自适应制动回中"),
    QuickCalStep("M03", "磁标定", "Yaw 负向单侧往返", "Tool Rx 以 10.5°/s 从 0° 到 -45°，实测外端制动行程后沿原路径回到 0°", "从标定中位开始；路径无碰撞", 0, 0, 10, 0, 0x32, MCAL_CAPTURE_MAG, "掌部磁传感器原始场", "10 秒内完成负向单侧往返并自适应制动回中"),
